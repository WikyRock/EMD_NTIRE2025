...
class ChannelAttentionBlock(nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        attn = (q @ v.transpose(-2, -1)) * self.temperature
        attn = attn.softmax(dim=-1)
        x = (attn @ v)
...
class CrossModalAttention(nn.Module):
    def __init__(self, dim, num_heads, LayerNorm_type='WithBias'):
        ...
        self.num_heads = num_heads
        self.norm_short = LayerNorm(dim, LayerNorm_type)
        self.norm_long = LayerNorm(dim, LayerNorm_type)
        self.cab = ChannelAttentionBlock(dim, self.num_heads)
        # self.norm2 = LayerNorm(dim, LayerNorm_type)
        # self.ffb = FeedForwardBlock(dim, LayerNorm_type)
        self.norm1 = nn.LayerNorm(dim)
        self.ffb1 = Mlp(dim, 4*dim)
        self.norm2 = nn.LayerNorm(dim)
        self.ffb2 = Mlp(dim, 4*dim)

    def forward(self, x1, x2):
        b,c,h,w = x1.shape
        x1 = self.norm_short(x1)
        x2 = self.norm_long(x2)

        y1 = x1 + self.cab(x1, x2)
        y1 = to_3d(y1)
        y1 = y1 + self.ffb1(self.norm1(y1))
        y1 = to_4d(y1, h, w)

        y2 = x2 + self.cab(x2, x1)
        y2 = to_3d(y2)
        y2 = y2 + self.ffb2(self.norm2(y2))
        y2 = to_4d(y2, h, w)
        return y1, y2
    
class SameModalAttention(nn.Module):
    def __init__(self, dim, num_heads, LayerNorm_type='WithBias'):
        super(SameModalAttention, self).__init__()
        self.num_heads = num_heads
