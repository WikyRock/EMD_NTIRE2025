...
class DualCrossMamba_list(nn.Module):
    def __init__(self, dim,layer =1, LayerNorm_type='WithBias',Residual = True):
...
class DCM_Unet(nn.Module):
    def __init__(self, 
        ...
        super(DCM_Unet, self).__init__()

        self.patch_embed_b = OverlapPatchEmbed(inp_channels, dim)
        self.patch_embed_e = OverlapPatchEmbed(eve_channels, dim)

        # self.residual_level1_b = ResidualBlock(dim,dim)
        # self.residual_level1_e = ResidualBlock(dim,dim)
        self.encoder_level1  = DualCrossMamba_list(dim = dim*2**0, layer = num_blocks[0])
        #self.encoder_level1 = nn.Sequential(*[ DualCrossMamba(dim = dim ) for i in range(num_blocks[0])])
        
        self.down1_2_b = Downsample(dim) ## From Level 1 to Level 2
        self.down1_2_e = Downsample(dim) 
        self.encoder_level2  = DualCrossMamba_list(dim = dim*2**1, layer = num_blocks[1])
        #self.encoder_level2 = nn.Sequential(*[ DualCrossMamba(dim = dim*2**1 ) for i in range(num_blocks[1])])
        ...
        #self.encoder_level3 = nn.Sequential(*[ DualCrossMamba(dim = dim*2**2 ) for i in range(num_blocks[2])])
        ...
        #self.encoder_level4 = nn.Sequential(*[ DualCrossMamba(dim = dim*2**3 ) for i in range(num_blocks[3])])
        ...
        #self.decoder_level3 = nn.Sequential(*[ DualCrossMamba(dim = int(dim*2**2) ) for i in range(num_blocks[2])])
        ...
        self.decoder_level2 = DualCrossMamba_list(dim = dim*2**1, layer = num_blocks[1])
        #self.decoder_level2 = nn.Sequential(*[ DualCrossMamba(dim = int(dim*2**1) ) for i in range(num_blocks[1])])
        ...
        self.decoder_level1 = DualCrossMamba_list(dim = dim*2**1, layer = num_blocks[0])
        #self.decoder_level1 = nn.Sequential(*[ DualCrossMamba(dim = dim ) for i in range(num_blocks[0])])
